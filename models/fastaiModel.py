...
from models.resnet1d import resnet1d18, resnet1d34, resnet1d50
from models.timeseries_utils import TimeseriesDatasetCrops, ToTensor


class metric_func(Callback):
    "Obtains score using user-supplied function func (potentially ignoring targets with ignore_idx)"

    def __init__(self, func, name="metric_func", ignore_idx=None, one_hot_encode_target=True, argmax_pred=False,
...
def fmax_metric(targs, preds):
    return utilsClass.evaluate_experiment(targs, preds)["Fmax"]


def auc_metric(targs, preds):
    return utilsClass.evaluate_experiment(targs, preds)["macro_auc"]
...
class fastai_model(ClassificationModel):
    def __init__(self, name, n_classes, freq, outputfolder, input_shape, pretrained=False, input_size=2.5,
        ...
        if pretrained == True:
            if (pretrainedfolder is None):
                ...
                # pretrainedfolder = Path('/content/drive/MyDrive/ModelData/output/exp0/models/'+name.split("_pretrained")[0]+'/')
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if (self.pretrainedfolder is None):  # from scratch
            ...
            learn.model.apply(basic_conv1d.weight_init)
            ...
            if (self.loss == "nll_regression" or self.loss == "mse"):
                ...
                output_layer_new.apply(fastaiModel.nll_regression_init)
                learn.model.set_output_layer(output_layer_new)

            fastaiModel.lr_find_plot(learn, self.outputfolder)
            learn.fit_one_cycle(self.epochs, self.lr)  # slice(self.lr) if self.discriminative_lrs else self.lr)
            fastaiModel.losses_plot(learn, self.outputfolder)
        else:  # finetuning
            print("Finetuning...")
            # create learner
            ...
            if (self.gradual_unfreezing):
                ...
                learn.freeze()
                fastaiModel.lr_find_plot(learn, self.outputfolder, "lr_find0")
                learn.fit_one_cycle(self.epochs_finetuning, lr)
                fastaiModel.losses_plot(learn, self.outputfolder, "losses0")
            ...
            learn.unfreeze()
            fastaiModel.lr_find_plot(learn, self.outputfolder, "lr_find" + str(len(layer_groups)))
            learn.fit_one_cycle(self.epochs_finetuning, slice(lr / 1000, lr / 10))
            fastaiModel.losses_plot(learn, self.outputfolder, "losses" + str(len(layer_groups)))
    ...
    def predict(self, X):
        ...
        return timeseries_utils.aggregate_predictions(preds, idmap=idmap,
    ...
    def _get_learner(self, X_train, y_train, X_val, y_val, num_classes=None):
        ...
        print("model:",
              self.name)  # note: all models of a particular kind share the same prefix but potentially a different postfix such as _input256
        ...
        if (self.name.startswith("fastai_resnet1d18")):
            ...
            model = resnet1d.resnet1d101(num_classes=num_classes, input_channels=self.input_channels, inplanes=128,
                                         kernel_size=self.kernel_size, ps_head=self.ps_head,
                                         lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_resnet1d152")):
            model = resnet1d.resnet1d152(num_classes=num_classes, input_channels=self.input_channels, inplanes=128,
                                         kernel_size=self.kernel_size, ps_head=self.ps_head,
                                         lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_resnet1d_wang")):
            model = resnet1d.resnet1d_wang(num_classes=num_classes, input_channels=self.input_channels,
                                           kernel_size=self.kernel_size, ps_head=self.ps_head,
                                           lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_wrn1d_22")):
            model = resnet1d.wrn1d_22(num_classes=num_classes, input_channels=self.input_channels,
                                      kernel_size=self.kernel_size, ps_head=self.ps_head,
                                      lin_ftrs_head=self.lin_ftrs_head)
        ...
        # passing the default kernel size of 5 leads to a max kernel size of 40-1 in the inception model as proposed in the original paper
            ...
            model = inception1d.inception1d(num_classes=num_classes, input_channels=self.input_channels,
        ...
        # basic_conv1d fcn,fcn_wang,schirrmeister,sen,basic1d
            ...
            model = basic_conv1d.fcn_wang(num_classes=num_classes, input_channels=self.input_channels,
                                          ps_head=self.ps_head, lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_fcn")):
            model = basic_conv1d.fcn(num_classes=num_classes, input_channels=self.input_channels)
        elif (self.name.startswith("fastai_schirrmeister")):
            model = basic_conv1d.schirrmeister(num_classes=num_classes, input_channels=self.input_channels,
                                               ps_head=self.ps_head, lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_sen")):
            model = basic_conv1d.sen(num_classes=num_classes, input_channels=self.input_channels, ps_head=self.ps_head,
                                     lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_basic1d")):
            model = basic_conv1d.basic1d(num_classes=num_classes, input_channels=self.input_channels,
                                         kernel_size=self.kernel_size, ps_head=self.ps_head,
                                         lin_ftrs_head=self.lin_ftrs_head)
        # RNN
        elif (self.name.startswith("fastai_lstm_bidir")):
            model = rnn1d.RNN1d(input_channels=self.input_channels, num_classes=num_classes, lstm=True,
                                bidirectional=True, ps_head=self.ps_head, lin_ftrs_head=self.lin_ftrs_head)
        elif (self.name.startswith("fastai_gru_bidir")):
            model = rnn1d.RNN1d(input_channels=self.input_channels, num_classes=num_classes, lstm=False,
                                bidirectional=True, ps_head=self.ps_head, lin_ftrs_head=self.lin_ftrs_head)
        ...
        if (self.early_stopping is not None):
            ...
            if (self.early_stopping == "macro_auc" and self.loss != "mse" and self.loss != "nll_regression"):
                metric = fastaiModel.metric_func(fastaiModel.auc_metric, self.early_stopping,
                ...
                metric = fastaiModel.metric_func(fastaiModel.fmax_metric, self.early_stopping,
                                                 one_hot_encode_target=False, argmax_pred=False, softmax_pred=False,
                                                 sigmoid_pred=True, flatten_target=False)
                learn.metrics.append(metric)
                learn.callback_fns.append(
