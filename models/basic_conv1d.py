...
def _conv1d(in_planes, out_planes, kernel_size=3, stride=1, dilation=1, act="relu", bn=True, drop_p=0):
    ...
    if (bn):
        lst.append(nn.BatchNorm1d(
            out_planes))  # Batch Normalization allows us to use much higher learning rates and be less careful about initialization.
    if (act == "relu"):
...
def attrib_adaptiveconcatpool(self, relevant, irrelevant):
    return basic_conv1d.cd_adaptiveconcatpool(relevant, irrelevant, self)
...
class AdaptiveConcatPool1d(nn.Module):
    ...
    def attrib(self, relevant, irrelevant):
        return basic_conv1d.attrib_adaptiveconcatpool(self, relevant, irrelevant)
...
def weight_init(m):
    ...
    if isinstance(m, basic_conv1d.SqueezeExcite1d):
...
def create_head1d(nf: int, nc: int, lin_ftrs: Optional[Collection[int]] = None, ps: Floats = 0.5,
    ...
    layers = [basic_conv1d.AdaptiveConcatPool1d() if concat_pooling else nn.MaxPool1d(2), Flatten()]
...
class basic_conv1d(nn.Sequential):
    ...
    def __init__(self, filters=[128, 128, 128, 128], kernel_size=3, stride=2, dilation=1, pool=0, pool_stride=1,
        ...
        for i in range(len(filters)):
            ...
            layers_tmp.append(basic_conv1d._conv1d(input_channels if i == 0 else filters[i - 1], filters[i],
                                                   kernel_size=kernel_size[i],
                                                   stride=(1 if (split_first_layer is True and i == 0) else stride),
                                                   dilation=dilation, act="none" if (
                            (headless is True and i == len(filters) - 1) or (
            ...
            if ((split_first_layer is True and i == 0)):
                layers_tmp.append(
                    basic_conv1d._conv1d(filters[0], filters[0], kernel_size=1, stride=1, act=act, bn=bn, drop_p=0.))
            ...
            if (squeeze_excite_reduction > 0):
                layers_tmp.append(basic_conv1d.SqueezeExcite1d(filters[i], squeeze_excite_reduction))
        ...
        if (headless is True):
            ...
            head = basic_conv1d.create_head1d(filters[-1], nc=num_classes, lin_ftrs=lin_ftrs_head, ps=ps_head,
...
def fcn(filters=[128] * 5, num_classes=2, input_channels=8):
    ...
    return basic_conv1d(filters=filters_in, kernel_size=3, stride=1, pool=2, pool_stride=2,
...
def fcn_wang(num_classes=2, input_channels=8, lin_ftrs_head=None, ps_head=0.5, bn_final_head=False, bn_head=True,
             act_head="relu", concat_pooling=True):
    return basic_conv1d(filters=[128, 256, 128], kernel_size=[8, 5, 3], stride=1, pool=0, pool_stride=2,
...
def schirrmeister(num_classes=2, input_channels=8, lin_ftrs_head=None, ps_head=0.5, bn_final_head=False, bn_head=True,
                  act_head="relu", concat_pooling=True):
    return basic_conv1d(filters=[25, 50, 100, 200], kernel_size=10, stride=3, pool=3, pool_stride=1,
...
def sen(filters=[128] * 5, num_classes=2, input_channels=8, squeeze_excite_reduction=16, drop_p=0., lin_ftrs_head=None,
        ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True):
    return basic_conv1d(filters=filters, kernel_size=3, stride=2, pool=0, pool_stride=0, input_channels=input_channels,
...
def basic1d(filters=[128] * 5, kernel_size=3, stride=2, dilation=1, pool=0, pool_stride=1, squeeze_excite_reduction=0,
            num_classes=2, input_channels=8, act="relu", bn=True, headless=False, drop_p=0., lin_ftrs_head=None,
            ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True):
    return basic_conv1d(filters=filters, kernel_size=kernel_size, stride=stride, dilation=dilation, pool=pool,
