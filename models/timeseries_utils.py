...
def apply_butter_filter(data, filter,
                        forwardbackward=True):  # The function provides options for handling the edges of the signal.
...
def reformat_as_memmap(df, target_filename, data_folder=None, annotation=False, delete_npys=False):
    ...
    timeseries_utils.npys_to_memmap(npys_data, target_filename, delete_npys=delete_npys)
    if (annotation):
        timeseries_utils.npys_to_memmap(npys_label, target_filename.parent / (target_filename.stem + "_label.npy"),
    ...
    def __getitem__(self, idx):
        ...
        if (self.random_crop):  # random crop
            if (timesteps == self.output_size):
                ...
                start_idx_crop = start_idx + random.randint(0,
...
class ButterFilter(object):
    ...
    def __init__(self, lowcut=50, highcut=50, fs=100, order=5, btype='band', forwardbackward=True, input=True):
        self.filter = timeseries_utils.butter_filter(lowcut, highcut, fs, order, btype)
        self.input = input
